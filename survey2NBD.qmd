---
title: "Survey Data to NDD-Dirichlet"
author: "Hume Winzar"
...
Although the NBD–Dirichlet model ideally uses **panel data** 
(exact counts of purchases per brand per consumer), it is still possible to 
approximate the required inputs using **survey-style self-report data**.  
...
Surveys often collect behavioural data in simplified or banded form, 
for example:
...
My preference is to get exact numbers from respondents, but often this is 
infeasible, and respondents can give only a _best guess_ of such behaviour.
...
3. **Brand purchase proportions**  
   - “Of all your purchases in this category, approximately 
   what **percentage** went to each brand?”  
     (Must sum to 100%.)
...
While imperfect, these can be transformed into approximate
**purchase counts**, enabling useful NBD–Dirichlet teaching and exercises.
...
If we only have banded responses, then we can map each band to an approximate 
numeric value.
...
From these converted counts, compute:
- Mean category frequency $\mu$  
...
Then we can multiply by their estimated category frequency to 
obtain **pseudo-counts**:
...
This produces reasonable approximations of:
- Brand shares  
- Penetrations  
...
- Estimate $\alpha_0$ from duplication or adopt a reasonable value (5–40)
...
Even approximate pseudo-counts can provide:
- Category frequency distribution → NBD  
- Brand shares and duplication → Dirichlet  
- Predicted penetrations, loyalty patterns, and growth effects  

Enough to support robust **diagnostic reasoning**, and 
simplified **market simulations**.
...
This matters because the NBD uses the proportion of consumers who 
made **zero** purchases in the observation period:
...
- **Dispersion parameter $k$** (solved numerically)
  ...
  - Goal Seek (as in the extant _Excel_ worksheet)  
  - Newton–Raphson method  
  - Grid search  

These computed values ($\mu$, $P(0)$, and $k$) fully specify 
the **NBD component** of the NBD–Dirichlet model.
...
> Include only those consumers who have purchased the category 
...
Individuals who *never* buy the category:
- Should **not** be included in the NBD population  
...
This is expected:
- A 4-week period may have 40–60% zero-buyers  
- A 12-month period usually has very few  

The choice of period should match:
- the category purchase cycle  
- the analytical purpose  
- standardisation across brands

### D. Infrequent buyers are legitimate zero-buyers  

Consumers who buy the category occasionally but **did not buy this period** 
are *exactly* the people the NBD is designed to model.
...
- Use the resulting frequency distribution to compute $\mu$ and solve for $k$.  
:::
