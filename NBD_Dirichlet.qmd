...
It assumes:\
- Each individual has their own average purchase rate, *lambda*, (
$\lambda_i$ ),\
- Across people, ( $\lambda_i$ ) follows a *Gamma distribution*
($\Gamma(\cdot)$),\
- Purchases for each individual follow a *Poisson process*.

Mixing the two distributions yields the *Negative Binomial*.
...
Once we know how often people buy the category, we can model *which
brand* they buy.
...
-   Buyer behaviour differences largely reflect **market share and
...
Suppose a category has **four brands** with the following observed market shares:
...
The constant $\alpha_0$ is called the **concentration parameter** (or sometimes the “total precision” or “Dirichlet strength”).  
It represents how *tightly concentrated* the brand-choice probabilities are around the observed market shares.

- A **larger $\alpha_0$** means **less variability** across consumers (everyone buys in proportion to market shares).  
- A **smaller $\alpha_0$** means **more heterogeneity** (some buyers specialise strongly in one brand).

The value of $\alpha_0 = 20$ here is **not derived from the shares themselves** — it is either:

1. **Chosen arbitrarily** for illustration (to make the numbers round), or  
2. **Estimated from category data**, e.g., using observed duplication or repeat-buying rates.

In practice, researchers often infer $\alpha_0$ by fitting the Dirichlet model to panel data.  
For example, if the observed duplication of purchase (brand overlap) is high, that implies a **larger $\alpha_0$**; if duplication is low (buyers are more brand-loyal), $\alpha_0$ is smaller.
...
**Rule of thumb.**  \
Think **vertical** for $k$ (how many category purchases) and **horizontal** for $\alpha_0$ (how those purchases are shared across brands). 
...
Even bright business students sometimes confuse the Dirichlet model with psychological or managerial theories of consumer behaviour.  
...
\quad \text{where } \sum_{i=1}^m p_i = 1, \; p_i > 0
$$
...
**without needing to know that shopper’s individual loyalties
explicitly**. The $\alpha_i$ encode typical brand attractiveness /
...
-   Sharp, B., Kennedy, R., & Sharp, A. (2019).
...
-   Sharp, B., Wright, M., & Goodhardt, G. J. (2002).
    Purchase loyalty is polarised, how brand loyalty differs by customer size.
...
-   Ehrenberg, A. S. C., Uncles, M. D., & Goodhardt, G. J. (2004).
    Understanding brand performance measures: Using Dirichlet distributions.
    *Journal of Business Research*, 57(12), 1307–1325.
